...
class SaleOrderReturn(models.Model):
    ...
    knk_sale_order_id = fields.Many2one(
        "sale.order",
        string="Return of",
        required=True,
        domain="[('partner_id', '=', partner_id)]",
    )
    knk_sale_order_return_line_ids = fields.One2many(
    ...
    state = fields.Selection(
        [("draft", "Draft"), ("confirm", "Return Order")], default="draft"
    )
    ...
    incoming_count = fields.Integer(
        string="Incoming shipments", compute="_compute_picks"
    )
    ...
    def _get_sales_details(self):
        values = []
        if (
            self.knk_sale_order_id
            and self.knk_sale_order_id.delivery_status != "pending"
        ):
            values = {
            ...
            self.update(values)
            values = []
            for line in self.knk_sale_order_id.order_line:
                values.append(
                    [
                        0,
                        0,
        ...
        self.write({"knk_sale_order_return_line_ids": False})
        self.write({"knk_sale_order_return_line_ids": values})
    ...
    def button_confirm(self):
        ...
        for ind in range(len(self.knk_sale_order_return_line_ids)):
            if (
                self.knk_sale_order_return_line_ids[ind].knk_product_qty
                > self.knk_sale_order_id.order_line[ind].qty_delivered
                - self.knk_sale_order_id.order_line[ind].knk_return_qty
            ):
                raise UserError(
        ...
        if picking_type_id:
            res = self._prepare_picking(picking_type_id)
            new_picking_id = self.env["stock.picking"].create(res)
            self.knk_sale_order_id.knk_picking_ids = [
                (4, new_picking_id.id, 0)
            ]
            self.knk_picking_ids = [(4, new_picking_id.id, 0)]
            lines = self.knk_sale_order_return_line_ids
            for ind in range(len(lines)):
                sale_ln_id = (
                    lines[ind]
                    .knk_sale_return_id.knk_sale_order_id.order_line[ind]
                    .id
                )
                move_vals = self._prepare_stock_moves(
                    lines[ind], picking_type_id, new_picking_id, sale_ln_id
                )
    ...
    def _prepare_stock_moves(self, line, picking_type_id, picking, s_line_id):
        res = []
        partner = self.knk_sale_order_id.partner_id
        location_id = partner.property_stock_customer.id
        location_dest_id = picking_type_id.default_location_dest_id.id
        template = {
            ...
            "partner_id": partner.id,
            "state": "draft",
            "sale_line_id": s_line_id,
            "company_id": self.knk_sale_order_id.company_id.id,
            "picking_type_id": picking_type_id.id,
            "route_ids": picking_type_id.warehouse_id
            and [
                (6, 0, [x.id for x in picking_type_id.warehouse_id.route_ids])
            ]
            or [],
