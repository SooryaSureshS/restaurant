odoo.define('mask_cutomization.mask_load', function (require) {
'use strict';
...
publicWidget.registry.mask_load = publicWidget.Widget.extend({
    ...
    render_3d_mask: function (data){
        ...
        var url_image = window.location.origin+'/web/image?model=sale.order.line&id='+line_id+'&field=upload_your_image'
...
//                                    root.getObjectByName(data['main_mask_material_name']).material.map.repeat.y =1.3;
                            ...
                            if (area == 'logo') {
                                    ...
                                    if (root.getObjectByName(data['main_mask_material_name'])){
                                        root.getObjectByName(data['main_mask_front']).material.color.setHex( s );
                                    }else{
                            ...
                            if (area == 'blank'){
                                var s = "00test";
                                    s = mask_color.replace(/^#+/, "0x");
                                    if (root.getObjectByName(data['main_mask_material_name'])){
                                        root.getObjectByName(data['main_mask_material_name']).material.color.setHex( s );
                                    }else{
                                        console.log("main_mask_material_name not found on root object")
                                    }
                            }
