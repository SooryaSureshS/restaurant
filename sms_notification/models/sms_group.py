...
class SmsGroup(models.Model):
    ...
    def onchange_member_type(self):
        ...
        if self.member_type == 'customer':
            ...
            customers = self.env['res.partner'].sudo().search([('customer_rank', '>', 0)])
            self.sudo().write({'member_ids': customers.ids})
        if self.member_type == 'supplier':
            res['domain'] = {'member_ids': [('supplier_rank', '>', 0)]}
            suppliers_rank = self.env['res.partner'].sudo().search([('supplier_rank', '>', 0)])
            self.sudo().write({'member_ids': suppliers_rank.ids})
        if self.member_type == 'any':
            res['domain'] = {'member_ids': []}
            all_contacts = self.env['res.partner'].sudo().search([])
            self.sudo().write({'member_ids': all_contacts.ids})
        return res
