        ...
        if request.env['res.users'].browse(uid).has_group('base.group_user'):
            ...
            if user.kitchen_screen_user == 'cook' and user.default_pos:
                try:
                    ...
                    sessions = user.default_pos.session_ids.filtered(lambda r: r.user_id == user.id and r.state not in ['opened','opening_control'])
                    if not sessions:
...
class PickUpAndDelivery(http.Controller):
    ...
    @http.route('/get/time/cooking/curb', csrf=False, type='json', auth="public")
    ...
    def OrderPickupTime(self, **kw):
        ...
        if pickup_date and pickup_time and order_id:
            if order_id:
                ...
                if order:
                    order.sudo().write(
                        {'pickup_date': pickup_date_time, 'pickup_date_string': str(pickup_date_string), 'carrier_id': 1})
    ...
    def OrderTimeRange(self, **kw):
        ...
        while True:
            ...
            if float(n):
                ...
                if time_vals[0] and time_vals[1]:
                    # actual_time = next_date.replace(hour=int(time_vals[0]), minute=int(time_vals[1]), second=0)
                    actual_time = next_date + timedelta(hours=int(time_vals[0]), minutes=int(time_vals[1]), seconds=0)
                        ...
                        if time_from_data == '00:00' or time_to_data == '00:00':
                            ...
                            if time_from_data == '00:00' or time_to_data == '00:00':
                                ...
                                if time_from_data == '00:00' or time_to_data == '00:00':
                                    ...
                                    day = 'Friday'
                                    if time_from_data == '00:00' or time_to_data == '00:00':
                                        time_from_data = '{0:02.0f}:{1:02.0f}'.format(*divmod(float(saturday_from) * 60, 60))
                                        time_to_data = '{0:02.0f}:{1:02.0f}'.format(*divmod(float(saturday_to) * 60, 60))
                                        day = 'Saturday'
                                        if time_from_data == '00:00' or time_to_data == '00:00':
                                            time_from_data = '{0:02.0f}:{1:02.0f}'.format(*divmod(float(sunday_from) * 60, 60))
