odoo.define('backend_gant.GanttRow', function (require) {
"use strict";
...
var GanttRow = Widget.extend({
    ...
    init: function (parent, pillsInfo, viewInfo, options) {
        ...
        this.CustomAllTotalRow = parent.CustomAllTotalRow || false;
        if(pillsInfo.CustomAllTotal && pillsInfo.CustomAllTotal == 'groupHours'){
            this.weekend_total = true;
            this.groupHours = true;
        }
        this.groupCost = false;
        if(pillsInfo.CustomAllTotal && pillsInfo.CustomAllTotal == 'groupCost'){
            this.weekend_total = true;
            this.groupCost = true;
        }
        this.groupShifts = false;
        if(pillsInfo.CustomAllTotal && pillsInfo.CustomAllTotal == 'groupShifts'){
            this.weekend_total = true;
            this.groupShifts = true;
        }
        ...
        if (this.isTotal) {
            if(this.groupHours || this.groupCost || this.groupShifts){
    ...
    setDroppable: function (firstCell) {
        ...
        this.$el.droppable({
            drop: function (event, ui) {
                ...
                if (diff || (self.groupId !== oldGroupId)) {
    ...
    _aggregateGroupedPills: function () {
        ...
        if(self.weekend_total){
            console.log('=====this.pills========',this.pills,this)
            if(self.groupShifts){
                var weekend_total_text = 0;
                _.each(this.pills, function(w){
                   weekend_total_text += w.count;
                });
                self.weekend_total_text = weekend_total_text;
            }
            if(self.groupHours){
                var weekend_total_text = 0;
                _.each(this.pills, function(w){
                    _.each(w.aggregatedPills, function(e){
                        weekend_total_text += e.allocated_hours;
                    });
                });
                self.weekend_total_text = weekend_total_text.toFixed(2);
            }
            if(self.groupCost){
                var weekend_total_text = 0;
                var currency_symbol = '$';
                _.each(this.pills, function(w){
                    _.each(w.aggregatedPills, function(e){
                        weekend_total_text += e.allocated_hours * e.employee_timesheet_cost;
                        currency_symbol = e.currency_symbol;
                    });
                });
                self.weekend_total_text = currency_symbol + ' ' +weekend_total_text.toFixed(2);
            }
            console.log('========weekend_total===',)
        }
        ...
        this.pills.forEach(function (pill) {
            ...
            if (self.consolidate && self.consolidationParams.maxValue) {
                ...
                if(self.groupHours){
                    var hours_display_name = 0;
                    _.each(pill.aggregatedPills, function(w){
                       hours_display_name += w.allocated_hours;
                    });
                    pill.custom_field_display_name = hours_display_name.toFixed(2);
                }
                if(self.groupCost){
                    var cost_display_name = 0;
                    var currency_symbol = '$';
                    _.each(pill.aggregatedPills, function(w){
                        cost_display_name += w.allocated_hours * w.employee_timesheet_cost;
                        currency_symbol = w.currency_symbol;
                    });
                    pill.custom_field_display_name = currency_symbol + ' ' +cost_display_name.toFixed(2);
                }
    ...
    _setDraggable: function ($pill) {
        ...
        if (this.options.canEdit && !pill.disableStartResize && !pill.disableStopResize && !this.isGroup) {
            ...
            if (!this.$containment) {
                this.$containment = $('#o_contain_gant');
            }
            $pill.draggable({
