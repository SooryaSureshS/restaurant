odoo.define('backend_gant.GanttRenderer', function (require) {
"use strict";

var AbstractRenderer = require('web.AbstractRenderer');
var config = require('web.config');
var core = require('web.core');
var GanttRow = require('backend_gant.GanttRow');
...
var GanttRenderer = AbstractRenderer.extend({
    ...
    init: function (parent, state, params) {
        ...
        this.CustomAllTotalRow = params.CustomAllTotalRow;
    ...
    async _renderView() {
        ...
        var totalRow;
        var CustomAllTotalHours;
        var CustomAllTotalCost;
        if (this.totalRow) {
            totalRow = this._renderTotalRow();
        }


        if(this.CustomAllTotalRow){
            CustomAllTotalHours = this._renderCustomAllTotalHours();
            CustomAllTotalCost = this._renderCustomAllTotalCost();
        }
        ...
        return Promise.all(proms).then(function () {
            ...
            const $containment = $('<div id="o_contain_gant"/>');
            ...
            if (CustomAllTotalHours) {
                CustomAllTotalHours.$el.appendTo(self.$('.o_gantt_total_row_container'));
            }
            if (CustomAllTotalCost) {
                CustomAllTotalCost.$el.appendTo(self.$('.o_gantt_total_row_container'));
            }
    ...
    _renderTotalRow: function () {
        if(this.CustomAllTotalRow){
            var pillsInfo = {
                groupId: "groupTotal",
                CustomAllTotal: "groupShifts",
                pills: this.state.records,
                groupLevel: 0,
                groupName: "Total Shifts"
            };
        }else{
            var pillsInfo = {
                groupId: "groupTotal",
    ...
    _renderCustomAllTotalHours: function () {
        var pillsInfo = {
            groupId: "groupTotal",
            CustomAllTotal: "groupHours",
            pills: this.state.records,
            groupLevel: 0,
    ...
    _renderCustomAllTotalCost: function () {
        var pillsInfo = {
            groupId: "groupTotal",
            CustomAllTotal: "groupCost",
            pills: this.state.records,
            groupLevel: 0,
            groupName: "Cost of Labour"
        };
        var params = {
            canCreate: this.canCreate,
            canCellCreate: this.canCellCreate,
    ...
    _onKeydown: function (ev) {
        ...
        if (this.$draggedPill && this.action === 'copy') {
            this.$el.addClass('o_copy');
            this.$el.removeClass('o_grab');
        }
    },
    ...
    _onKeyup: function (ev) {
        ...
        if (this.$draggedPill && this.action === 'reschedule') {
            this.$el.addClass('o_grab');
            this.$el.removeClass('o_copy');
        }
    },

    _onStartDragging: function (event) {
        this.$draggedPill = event.data.$draggedPill;
        this.$draggedPill.addClass('o_pill_drag');
        if (this.action === 'copy') {
            this.$el.addClass('o_copy');
        } else {
            this.$el.addClass('o_grab');
    ...
    _onStartNoDragging: function () {
        this.$el.addClass('o_draging');
    },

    _onStopDragging: function () {
        this.$draggedPill.removeClass('o_pill_drag');
        this.$draggedPill = null;
        this.$draggedPillClone = null;
        this.$el.removeClass('o_grab');
        this.$el.removeClass('o_copy');
