...
class OrderFetch(Controller):
    ...
    def order_fetch(self, session_id):
        ...
        # pos order
        pos_order_line_domain = ['cancel', 'return', 'done', 'draft']
        domain = [('lines.order_printed', '=', False),
                  ('lines.order_line_state', 'not in', ['cancel', 'done', 'return', False]),
        ...
        for i in pos_line:
            ...
            optional_line = []
            previous_line = False
            for line in i.lines.filtered(lambda r:  r.order_line_state not in pos_order_line_domain and r.order_printed == False):
            ...
            if len(list1) > 0:
                ...
                sorted_order_line = sorted(list1, key=lambda k: k['pos_categ_sequence'])
                ...
                for pos_cat  in config_id.printer_ids:
                    ...
                    if len(order_line_data)>0:
                        ...
                        #     for o in orders:
                        #         o.write({'order_printed': True})
                        po.append({pos_cat.id: pos_order})

        # sale order
        sale_domain = False
        search_domain = [('order_line.order_printed', '=', False),
        ...
        for i in so_line:
            ...
            if i.state == 'sale':
                ...
                selected_line = False
                for lines in i.order_line.filtered(lambda r: r.order_line_state not in ['cancel', 'return', 'done'] and r.product_id.is_optional_product==False and r.bundle_child == False and r.order_printed == False):
                    if lines:
                ...
                if len(list1) > 0:
                    ...
                    order = {
                        ...
                        'friendly_1': i.friendly_id.split("#")[0] if i.friendly_id else False,
                        'all_category': all_category,
                        'street': i.partner_id.street if i.partner_id else False,
                        'street2': i.partner_id.street2 if i.partner_id else False,
                        'city': i.partner_id.city if i.partner_id else False,
                    ...
                    for pos_cat in config_id.printer_ids:
                        ...
                        if len(order_line) > 0:
                            ...
                            ids = []
                            if 'order_line' in order_pos:
                                for j in order_pos['order_line']:
                                    ids.append(j['id'])
                                    for k in j['optional_line_pos']:
                                        ids.append(k['id'])
                            # if bool(ids):
    ...
    def order_print_update(self, order, types):

        if bool(order):
            print("ordersrrsrsrsrrssrsr", order,types)
            if 'order_line' in order:
                for i in order['order_line']:
                    print("bbbbbbbbbbb",i)
            # order = order[0]
            if types == 'pos':
                # order = request.env['pos.order'].sudo().browse(int(order['order_id']))
                # order.order_printed = True
                ids = []
                if order:
                    for i in order:
                        ids.append(i['id'])
                        for k in i['optional_line_pos']:
                            ids.append(k['id'])
                print("ordersrrsrsrsrrssrsr",ids)
                if bool(ids):
                    lines = request.env['pos.order.line'].sudo().browse(ids)
                    for o in lines:
                        print('orders iddsdsdsdsdsdsdsdsdsd',o.product_id.name)
                        o.write({'order_printed': True})
                        request.env.cr.commit()
            elif types == 'sale':
                # order = request.env['sale.order'].sudo().browse(int(order['order_id'][0]))
                # order.order_printed = True
                ids = []
                if order:
                    for i in order:
                        ...
                        for k in i['optional_line_pos']:
                            ids.append(k['id'])
                if bool(ids):
                    lines = request.env['sale.order.line'].sudo().browse(ids)
                    for o in lines:
                        print('orders iddsdsdsdsdsdsdsdsdsd', o.product_id.name)
                        o.write({'order_printed': True})
                        request.env.cr.commit()
