...
class SaleOrderNewInherited(models.Model):
    ...
    def action_recovery_email_sendNew(self):
        ...
        composer_form_view_id = self.env.ref('server_action_date_order.operational_report_wizard_form_news').id
        #
    ...
    def print_report_date(self):
        if self.date_orders_trans:
            for i in self.sale.strip('][').split(', '):
                orders = self.env['sale.order'].search([('id','=',int(i))],limit=1)
                for order in orders:
                    order.write({
