odoo.define('theme_clarico_vego.cart_popup', function(require) {
    ...
    publicWidget.registry.cart_popup = publicWidget.Widget.extend({
        selector: '#wrapwrap',
        read_events: {
            'click #add_to_cart_shop': 'addToCart',
            'click #add_to_cart': 'addToCart2',
        },

        init: function () {
            this._super.apply(this, arguments);
            ajax.jsonRpc("/shop/cart/quantity/custom", 'call', {}).then(function (data) {
                sessionStorage.setItem('website_sale_cart_quantity', data);
                if (data>0){
                    $('.my_cart_quantity').html(data);
                    $('li.o_wsale_my_cart').attr("style", "display: block !important");
                }
                else{
                    $('.my_cart_quantity').html(data);
                    $('li.o_wsale_my_cart').attr("style", "display: none !important");
                }
            });
        },
        addToCart2: function (ev){
            $('li.o_wsale_my_cart').attr("style", "display: block !important");
        },
        addToCart: function (ev){
            var productId = ev.currentTarget.getAttribute('data-pid');
            var quantity = 1;
            ajax.jsonRpc('/shop/cart/update/order', 'call', {
                'product_id': productId,
                'add_qty': quantity,
            }).then(function (data) {
                var current_qty = parseInt($('.my_cart_quantity').html())
                var new_qty = current_qty + 1
                $('.my_cart_quantity').html(new_qty);
                $('li.o_wsale_my_cart').attr("style", "display: block !important");
            });
        },
    ...
    publicWidget.registry.websiteSaleCartLink.include({
        ...
        this.$el.popover({
            ...
            container: 'body',
            placement: 'auto',
            template: '<div class="popover mycart-popover te_open" role="tooltip"><div class="tooltip-arrow"></div><h3 class="popover-header"></h3><div class="te_cross">X</div><div class="popover-body"></div></div>'
        });
